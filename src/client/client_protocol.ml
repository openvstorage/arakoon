...
let read_command conn =
  let ic = conn.ic and oc = conn.oc in
...
let _do_command conn (backend:Backend.backend) command =
  let ic = conn.ic
  and oc = conn.oc
  and id = conn.cid
  in
       ...
       wrap_exception
         (fun () ->
           ...
           (match HookRegistry.lookup name with
            | h ->
               h (ic,oc,id) (backend # get_read_user_db ()) (backend :> backend)
            | exception Not_found ->
...
let one_command stop (conn:Server.connection) (backend:Backend.backend) =
    read_command conn >>= fun command ->
  if !stop
  then
    Lwt.return_true
  else
    let () = Server.cork conn in
    _do_command conn backend command >>= fun death ->
    let () = Server.uncork conn in
    Lwt.return death



let protocol stop backend conn =
  let open Server in
  let ic = conn.ic
  and oc = conn.oc
  in
  ...
  let rec loop () =
    begin
      one_command stop conn backend >>= fun death ->
      Lwt_io.flush oc >>= fun() ->
      if death || !stop
