...
let retrieve = function
  ...
  | Arakoon { cluster_id; key; ini_location; } ->
     ...
     let cfg = Node_cfg._retrieve_cfg_from_txt ini_location in
     let ccfg = Node_cfg.to_client_cfg cfg ~ssl_cfg:None in (* TODO ssl/tls if needed? *)
...
let retrieve_cfg url = retrieve url >|= Node_cfg._retrieve_cfg_from_txt

let retrieve_client_cfg url = retrieve_cfg url >>= fun cfg -> Lwt.return (Node_cfg.to_client_cfg cfg)
